...
, function 第五个函数(t, e, i) {
    ...
    var s = i(0)
      ...
      , c = function() {
        ...
        ,
        t.prototype.spawnNewGuest = function(t) {
            void 0 === t && (t = null);
            var e = 400
              , i = s.Game.system.linearMap(3, 12);
            i < n.Player.instance.ySpeed && (e = s.Game.system.height),
    ...
    e.GameSetup = function() {
        var t = $("game-canvas")
          ...
          , n = function() {//在此以实现自定义效果
            ...
            socket.on('g-dir',(dir)=>{
                if (game.spawn.guests.length==0){
                    game.spawn.spawnNewSurfer()
                    game.spawn.guests.push(game.spawn.surfers[game.spawn.surfers.length-1])
                    game.spawn.guests[0].updateDirection=()=>{}
                
                }
                         game.spawn.guests[0].surferAngle=dir
                        }),
                        socket.on('delPlayer',()=>{
                            game.spawn.guests.pop()
                        }),
            i = window.performance.now())
        };
        requestAnimationFrame(n),
...
function 第十二个函数(t, e, i) {
    ...
    var r = i(0)
      ...
      , h = function(t) {
        ...
        ,
        e.prototype.update = function() {//更新sufer
            this.updateInterval(),
            // console.log(this)
            t.prototype.update.call(this),
        ...
        ,
        e.prototype.updateDirection = function() {//sufer方向
        ...
        ,  
        e.prototype.draw = function(e) {//surfer渲染
            // socket.on('g-update',(x,y,frame,pose)=>{
            // this.surferSprite.draw(e, Math.floor(x), Math.floor(y), frame, pose)
            // })
            // console.log('draw surfer')
...
function 第十三个函数(t, e, i) {
    ...
    var r = i(0)
      ...
      , c = function(t) {
        ...
        ,
        e.prototype.updatePosition = function() {//敌人方位
          ...
          , c = {
            ...
            onKeydownPlaying: function(e) {
                if (c.isDown(c.DOWN) || c.isDown(c.S)) {
                    ...
                    t = 0;
                c.isDown(c.LEFT) || c.isDown(c.A) ? "left-down" === r.Player.instance.state || "left" === 
                r.Player.instance.state ? 
                r.Player.instance.state = (()=>{socket.emit('dir',"left");return"left"})(): 
                r.Player.instance.state = (()=>{socket.emit('dir',"left");return"left-down"})() : c.isDown(c.RIGHT)
                 || c.isDown(c.D) ? "right-down" === r.Player.instance.state 
                 || "right" === r.Player.instance.state ?
                  r.Player.instance.state = (()=>{socket.emit('dir',"right");return"right"})() : 
                  r.Player.instance.state = (()=>{socket.emit('dir',"right");return"right-down"})() :
                   c.isDown(c.UP) || c.isDown(c.W) ? 
                   r.Player.instance.state = (()=>{socket.emit('dir',"stop");return"stop"})(): 
                   c.isDown(c.F) && r.Player.instance.boost()
            },
